# ALERTBEAT
ALERTBEAT's creational purpose is for sending alerts at regular intervals based on log monitor filters. However, ALERTBEAT can also be used to run other functions on the same schedule too (but cron and systemd exist for a reason). Thanks to the `importlib` library, the script automatically updates the running services and integrations used when it senses changes in the `handler.py` file without restarting the entire script. This allows you to add, remove, or update services and/or integrations without interrupting the main process.
...
A service defined in ALERTBEAT is a python script located in the root of the `services` folder. A service has the following minimum requirements:
 ...
 - A `send_message(self, message: dict)` function inside this class that returns a `requests.Response` if OK, `None` if no request was sent and no errors want to be raised, or an `Exception` if an error occurred - optional as the handler has a catch anyway.
An example `discord.py` service is included. This sends a webhook to the `WEBHOOKS['Discord']` url found in `.env` with an embed for the alert details and a separate embed for any JSON data.
